<template>
	<main class="w-full columnAlignCenter">
		<h2>Licencias</h2>
		<div v-if="!loading" class="w-full tabsLicencia">
			<TabView>
	...
	<Dialog class="dialogLicencias w-11" v-model:visible="dialogVisible">
		<div class="column gap-2 p-2" v-if="licenciaSeleccionada">
			...
			<template v-else-if="dialogType === 'aprobar'">
				<p class="mb-1">
					¿Estas seguro que queres aprobar esta licencia?
				</p>
				<p>
					<span class="titleDialog">Profesional:</span>
					{{ licenciaSeleccionada.fullname }}
			...
			<template v-else-if="dialogType === 'rechazar'">
				<p class="mb-1">
					¿Estas seguro que queres rechazar esta licencia?
				</p>
				<p>
					<span class="titleDialog">Profesional:</span>
					{{ licenciaSeleccionada.fullname }}
			...
			<template v-else-if="dialogType === 'editar'">
				...
				<div>
					<label for="tipoLicencia"
						>Edite el tipo de la licencia</label
					>
					<Dropdown
						id="tipoLicencia"
...
export default {
	...
	methods: {
		...
		async getProfessionalFullName(cuil) {
			const prof = await this.profStore
				.getProfesional(cuil)
				.catch((error) => {
					console.log(error);
				});
