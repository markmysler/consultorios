		...
		<Stepper class="stepperAgenda w-full h-full bg-gray">
			...
			<!-- <StepperPanel class="h-full columnSpaceBetween">
				<template #content="{ nextCallback }">
					<div>
						<label for=""
							>Busque un profesional por nombre o CUIL</label
						>
					...
					<div>
						<div class="rowSpaceBetween datosContainer py-2">
							<div class="datoLicencia column gap-1">
								<p>Profesional</p>
								<p class="datos text-blue" v-if="profesional">
									{{
										profesional.nombre +
										" " +
...
export default {
	...
	methods: {
		...
		async agregarLicencia() {
			this.serverError = null;
			this.loading = true;
			// Router push AddLicenciaConfirmation
			const licenciaRequest = {
				inicio: this.licenciaAgregada.fechaInicio,
				fin: this.licenciaAgregada.fechaFin,
				cuil: this.cuil,
				tipo: this.licenciaAgregada.tipoLicencia,
				anio: this.licenciaAgregada.anio,
				operacion: "creacion",
			};
			...
			await requestOrAddLicencia(licenciaRequest)
				.then((res) => {
					this.loading = false;
					console.log(res.data);

					if (res.data === "Licencia agregada") {
						this.router.push({
							path: ROUTES_NAMES.AddLicenciaConfirmation,
							query: {
								licenciaAgregada:
									JSON.stringify(licenciaRequest),
								nombreProfesional:
	...
	async mounted() {
		this.cuil = this.route.query.cuil;
		this.profesional = await this.profStore.getProfesional(this.cuil);
	},
};
</script>
